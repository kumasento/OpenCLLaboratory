...
void get_cl_build_program_log(cl_program program, cl_device_id device) {
    puts("");
...
int get_divided_value(int lower_bound, int div) {
    return (lower_bound % div == 0) ? 
            lower_bound :
            ((lower_bound/div + 1) * div);
}
